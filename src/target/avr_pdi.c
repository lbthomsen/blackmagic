...
static bool avr_flash_erase(struct target_flash *f, target_addr_t addr, size_t len);
static bool avr_flash_write(struct target_flash *f, target_addr_t dest, const void *src, size_t len);
...
static void avr_halt_resume(target *t, const bool step)
{
	...
	if (step)
	{
		const target_addr_t currentPC = pdi->programCounter;
		const target_addr_t nextPC = currentPC + 1U;
