...
retry:
	...
	if (dmi->idle >= 2 + 2)
		jtag_proc.jtagtap_tms_seq(0, dmi->idle - (1 + 2));
	...
	switch (DMI_GET_OP(dmi_ret)) {
		case DMISTAT_OP_BUSY:
			...
			DEBUG_WARN("RISC-V DMI op interrupted ret = 0x%"PRIx64"\n", dmi_ret);
...
void rvdbg013_jtag_dp_handler(uint8_t jd_index)
{
	uint32_t dtmcontrol;
	uint8_t version;
	...
	rvdbg_jtag->dp_jd_index = jd_index;
	rvdbg_jtag->dmi.idcode = jtag_devs[jd_index].jd_idcode;
